...
extern "C" __attribute__((weak)) void func_801D8D30(CpuContext* ctx);
extern "C" __attribute__((weak)) void func_801D9E94(CpuContext* ctx);

static uint32_t GetNHttpSystemInfo(CpuContext* ctx, uint32_t& savedR3, uint32_t& savedR4, uint32_t& savedR5) {
    savedR3 = ctx->gpr[3];
    savedR4 = ctx->gpr[4];
    savedR5 = ctx->gpr[5];
    WIINX_GUEST_HELPER(func_801D9E94, ctx);
...
extern "C" void NHTTPStartup_Reentrant_HLE_801d8d30(CpuContext* ctx) {
    ...
    ctx->gpr[4] = savedR4;
    ctx->gpr[5] = savedR5;
    WIINX_GUEST_HELPER(func_801D8D30, ctx);
